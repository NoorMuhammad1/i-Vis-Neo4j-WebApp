...
function App() {
  ...
  const renderGraph = (ref, data, mainActor) => {
    ref.elements().remove();
      ...
      if (ref.getElementById(node.name).length === 0) {
        ref.add({
          data: {
            group: node.label,
            id: node.name,
            main: node.name === mainActor ? "true" : "false",
            ...(node.label === "Person"
              ? {
                  born: node.born,
                }
        ...
        node.label === "Person" && n.push(node.name);
      }
    });
    edges.forEach((edge) => {
  ...
  const apiCall = async () => {
    ...
    try {
      ...
      renderGraph(myCyRef, data, request.actor);
    } catch (error) {
  ...
  const aStar = () => {
    if (bfs.from !== "" && bfs.to !== "") {
      const star = myCyRef.elements().aStar({
        root: `node[id="${bfs.from}"]`,
        goal: `node[id="${bfs.to}"]`,
      });
      star.path.style("line-color", "red");
    } else {
      alert("Aleast two nodes should be selected for path highlighting");
    }
  };
  ...
  const handleSelectChange = (event, property) => {
    ...
    cy.nodes().forEach((node) => {
      ...
      if (!(id === bfs.from || id === bfs.to || id === event.target.value))
  ...
  useEffect(() => {
    const cy = myCyRef;
    cy.contextMenus(contextOptions);
    cy.elements().bind("click", (event) => event.target.tippy.show());
    cy.on("mouseover", "node", (event) => {
      const node = event.target;
      node.connectedEdges().style({ "line-color": "red", width: "3px" });
      node
        .connectedEdges()
        .connectedNodes()
    ...
    cy.on("mouseout", "node", (event) => {
      const node = event.target;
      // Revert connected edges style
      node.connectedEdges().style({ "line-color": "gray", width: "1px" }); // Assuming '#95a5a6' is the original color
      node
        .connectedEdges()
        .connectedNodes()
        .forEach((n) => {
          n.style({
            "border-color": n.data("main") === "true" ? "blue" : "gray",
            "border-width": "2px",
          });
        });
    });
      ...
      >
        ...
        >
          ...
          >
            ...
            <button style={{ margin: "0.4em 0" }} onClick={aStar}>
              Select
            </button>
          </div>
        </div>
